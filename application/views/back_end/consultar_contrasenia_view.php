					...
					<form class="form-horizontal form-label-left input_mask" name="add_pass" id="add_pass" method="POST">
						<?php foreach ($clientes_data as $data_cte): ?>
							...
							<div class="col-md-12 col-sm-6 col-xs-12 form-group has-feedback">
								<i class="fa fa-key"></i>  CONTRASEÑAS REGISTRADAS A: <strong><?php echo strtoupper($data_cte->nombres) . ' ' . strtoupper($data_cte->ap_p) . ' ' . strtoupper($data_cte->ap_m); ?></strong>
							</div>
						<?php endforeach; ?>
						...
						<div class="col-md-3 col-sm-6 col-xs-12 ">
							<button type="button" class="btn btn-danger"  id="cancel_all"><i class="fa fa-home"></i> Consulta nueva  </button>
						</div>
...
<script type="text/javascript">
	$('#load_form_register_pass').click(function () {
		URL = base_url + "clientes/consulta_type_form_pass";
		$.ajax({
			type: "POST",
			url: URL,
